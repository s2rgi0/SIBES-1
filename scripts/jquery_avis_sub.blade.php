<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        ////ESTA FUNCION LLENA TODOS LOS SELECTS // CUANDO VAMOS AGREGAR AVISTAMIENTO
        ...
        $('#frm-avista button').click(function(e){
            ...
            if($(this).attr("id")=="guardar_link"){
                ...
                if( $('#id_PubPDF').val() ){
                    ...
                    var div = $('#link_esp_sub');
                    var esc = $('#esc_sub');
                    
                    div.find('#pub_PDF_sub').append(link);
                    esc.find('#publish_sub').append(link);

                    $("#id_PubPDF").val("");
                    
                    var espacio = '\n';
                    
                    div.find('#pub_PDF_sub').append(espacio);
                    //div.find('#pub_PDF2').append(espacio);
                    esc.find('#publish_sub').append(espacio);
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            if(data.success == false ){
                                ...
                                $('#_fuente_avis,#_num_avis,#_foto_graf,#_zona_avis,#_PDF-file,#_lati_avis,#_long_avis,#_long_min,#_long_sec,#_lati_min,#_lati_sec,#_col_avis,#_tierra_avis,#_suelo_avis,#_depar_avis,#_mun_avis,#_canton_avis,#_alt_avis,#_fecha_av,#_clima_avis,#_eco_avis,#_geo_avis,#_clima_avis,#_hidro_avis,#_fisio_Avis').text('');
                                ...
                                function(isConfirm){
                                if (isConfirm) {
                                    $('#PDF_avis_sub').modal('show');
                                } 
                                });

                                $.ajax({
                                type : 'get',
                                url  : '{!! URL::to('get_avista_SUB') !!}',
                                data : { 'id' : id },
                                success:function(data){
        ...
        function llenar_campos_avis( id, div ){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    div.find('#publicacion_PDF_sub').text(data[0].publicacionPdf);        
        ...
        $(document).on('click','.btn-Editar',function(e){
            ...
            //alert(id)
            console.log(id)
            $("#id_PubPDF_edit_sub").val("");
            $('#publish_edit_sub').empty();
            $('#pub_PDF_edit_sub').empty();            

            datos_avista( m_avis , id );
            colector( m_avis , id );
            fuenteInfo( m_avis , id );
            departamento( m_avis , id );
            municipio_avis( m_avis ,id );
...
function datos_avista( m_avis , id ){
    ...
    $.ajax({
                ...
                data :  { 'id':id },
                success:function(data){
                    console.log('funciono el get avistamiento con sus datos')
                    console.log(data)
                    m_avis.find('#id_esp_ver').val(data[0].idEspecie);
                    m_avis.find('#id_sub_ver').val(data[0].idSubespecie);
                    m_avis.find('#id_avis_ver').val(data[0].idAvistamiento);
                    m_avis.find('#nom_esp').val(data[0].nombreEspecie);
                    m_avis.find('#nom_sub').val(data[0].nombreSubespecie);                    
                    //m_avis.find('#idFInfo_ver').val(data[0].nombreFuente);
                    ...
                    m_avis.find('#hora_avis22').val(data[0].horaAvistamiento);
                },
                error:function(){
...
function municipio_avis( m_avis , id ){
...
function departamento( m_avis ,id ){
...
//////////////////////        GUARDA     DESPUES   DE     EDITAR       //////////////////////
    ...
    $('#frm-editar_sub button').click(function(e){
        e.preventDefault();
        //alert('se apreto un boton en editar')

        if($(this).attr("id")=="guardar_link_edit_sub"){

                //alert('boton guarda documentos ')
         
            if( $('#id_PubPDF_edit_sub').val() ){

                //alert('boton guarda pDF ');

        var m_avis = $('#modal-body-ver');
        var id = $('#id_avis_ver').val();
        console.log(id);  

        var form = document.querySelector('#frm-editar_sub');
        var formdata = new FormData(form);
        console.log(form)
        console.log(formdata)
        $.ajax({
                ...
                success:function(data){
                    ...
                    console.log(data.errors)
                     ...
                     if(data.success == false ){
                        ...
                        $('#_foto-graf_ver,#_lati_avis_ver,#_lati_min_ver,#_lati_sec_ver,#_long_avis_ver,#_long_min_ver,#_long_sec_ver,#_col_avis_ver,#_depar_avis_ver,#_mun_avis_ver,#_canton_avis_ver,#_alt_avis_ver,#_clima_avis_ver,#_eco_avis_ver,#_geo_avis_ver,#_clima_avis_ver,#_hidro_avis_ver,#_fisio_Avis_ver').text('');
                                //$('#avis-error').fadeIn();
                        $.each(data.errors , function(index,value){
                            $('#_'+index).fadeIn();
                            $('#_'+index).text(value);
