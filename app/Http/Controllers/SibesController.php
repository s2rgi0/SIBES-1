...
class SibesController extends Controller
{
    ...
    public function mostrar(Request $req)
    {
        ...
        $validator = Validator::make($req->all(), [
            ...
            'reino.required'    => 'seleccione un reino',
            'division.required' => 'seleccione una division',
            'clase.required'    => 'seleccione una clase',
            'orden.required'    => 'seleccione un orden',
            'familia.required'  => 'seleccione una familia',
        ...
        if ($esp != null) {
            ...
            $nc_sub = Nombrecomun::where('idSubespecie', $esp)->get();
            $tipo   = Clasesdetipo::all();

            $t_sub = Clasesdetipo::join('Subespecies', 'Clasesdetipo.idClaseDeTipo', '=', 'Subespecies.idClaseDeTipo')->select('ClasesDeTipo.nombreClaseDeTipo', 'ClasesDeTipo.idClaseDeTipo')->where('Subespecies.idSubespecie', $esp)->get();

            $a_sub = ApendiceCites::join('Subespecies', 'apendice_cites.idApendiceCITES', '=', 'Subespecies.idApendiceCITES')->select('apendice_cites.idApendiceCITES', 'apendice_cites.nombreApendiceCITES')->where('Subespecies.idSubespecie', $esp)->get();
            $p_sub = ProcedenciaEspecie::join('Subespecies', 'procedencia_especies.idProcedenciaDeLaEspecie', '=', 'Subespecies.idProcedenciaDeLaEspecie')->where('Subespecies.idSubespecie', $esp)->get();
            $c_sub = CategoriaMarn::join('Subespecies', 'categoria_marns.idCategoriaMARN', '=', 'Subespecies.idCategoriaMARN')->select('categoria_marns.idCategoriaMARN', 'categoria_marns.nombreCategoriaMARN')->where('idSubespecie', $esp)->get();
            $u_sub = CategoriaUICN::join('Subespecies', 'categoria_u_i_c_ns.idCategoriaUICN', '=', 'Subespecies.idCategoriaUICN')->select('categoria_u_i_c_ns.idCategoriaUICN', 'categoria_u_i_c_ns.nombreCategoriaUICN')->where('idSubespecie', $esp)->get();
            ...
            //echo "<script>alert('buscaremos especie');</script>";
            $esp1 = Especie::join('Generos', 'Especies.idGenero', '=', 'Generos.idGenero')->join('Familias', 'Generos.idFamilia', '=', 'Familias.idFamilia')->join('Ordens', 'Familias.idOrden', '=', 'Ordens.idOrden')->join('Clases', 'Ordens.idClase', '=', 'Clases.idClase')->join('Divisions', 'Clases.idDivision', '=', 'Divisions.idDivision')->join('Reinos', 'Divisions.idReino', '=', 'Reinos.idReino')->select('Especies.nombreEspecie', 'Especies.idEspecie', 'Generos.nombreGenero', 'Familias.nombreFamilia', 'Ordens.nombreOrden', 'Clases.nombreClase', 'Divisions.nombreDivision', 'Reinos.nombreReino', 'Especies.idGenero', 'Especies.nombreEnIngles', 'Especies.descripcionDelEjemplar','Especies.fotografiaEspecie')->where('idEspecie', $esp2)->get();

            $esp1_array = json_decode($esp1);
            //dd($esp1_array[0]->nombreGenero);

            $append = ApendiceCites::all();
            $proc   = ProcedenciaEspecie::all();
            $cat    = CategoriaMarn::all();
            $uicn   = CategoriaUICN::all();
            $nc_esp = Nombrecomun::where('idEspecie', $esp2)->get();
            $tipo   = Clasesdetipo::all();

            $t_esp = Clasesdetipo::join('Especies', 'Clasesdetipo.idClaseDeTipo', '=', 'Especies.idClaseDeTipo')->select('ClasesDeTipo.nombreClaseDeTipo', 'ClasesDeTipo.idClaseDeTipo')->where('Especies.idEspecie', $esp2)->get();
            $a_esp = ApendiceCites::join('Especies', 'apendice_cites.idApendiceCITES', '=', 'Especies.idApendiceCITES')->select('apendice_cites.idApendiceCITES', 'apendice_cites.nombreApendiceCITES')->where('Especies.idEspecie', $esp2)->get();
            $p_esp = ProcedenciaEspecie::join('Especies', 'procedencia_especies.idProcedenciaDeLaEspecie', '=', 'Especies.idProcedenciaDeLaEspecie')->where('Especies.idEspecie', $esp2)->get();
            $c_esp = CategoriaMarn::join('Especies', 'categoria_marns.idCategoriaMARN', '=', 'Especies.idCategoriaMARN')->select('categoria_marns.idCategoriaMARN', 'categoria_marns.nombreCategoriaMARN')->where('idEspecie', $esp2)->get();
            $u_esp = CategoriaUICN::join('Especies', 'categoria_u_i_c_ns.idCategoriaUICN', '=', 'Especies.idCategoriaUICN')->select('categoria_u_i_c_ns.idCategoriaUICN', 'categoria_u_i_c_ns.nombreCategoriaUICN')->where('idEspecie', $esp2)->get();

            return view('ingreso.reporte', compact('esp1_array', 'usuario', 'tipo', 't_esp', 'elementCount', 'append', 'proc', 'cat', 'uicn', 'nc_esp', 'a_esp', 'p_esp', 'c_esp', 'u_esp'));

        } else if (request('genero') == null) {

            //dd(request('genero'));

            echo "<script>alert('Debe elegir un genero');</script>";

            //sleep(0.25);

            return redirect('/Busqueda');

        } else if (request('familia') == null) {

            echo "<script>alert('Debe elegir una familia');</script>";

            return redirect('/Busqueda');

        } else if (request('orden') == null) {

            echo "<script>alert('Debe elegir un Orden');</script>";

            return redirect('/Busqueda');

        } else if (request('clase')) {

            echo "<script>alert('Debe elegir una Clase');</script>";

            return redirect('/Busqueda');

        } else if (request('division') == null) {

            echo "<script>alert('Debe elegir una division');</script>";

            return redirect('/Busqueda');

        } else if (request('reino') == null) {

            echo "<script>alert('Debe elegir un Reino');</script>";
    ...
    public function muestra_especie(Request $r)
    {

        $esp1 = Especie::join('Generos', 'Especies.idGenero', '=', 'Generos.idGenero')->join('Familias', 'Generos.idFamilia', '=', 'Familias.idFamilia')->join('Ordens', 'Familias.idOrden', '=', 'Ordens.idOrden')->join('Clases', 'Ordens.idClase', '=', 'Clases.idClase')->join('Divisions', 'Clases.idDivision', '=', 'Divisions.idDivision')->join('Reinos', 'Divisions.idReino', '=', 'Reinos.idReino')->select('Especies.nombreEspecie', 'Especies.idEspecie', 'Generos.nombreGenero', 'Familias.nombreFamilia', 'Ordens.nombreOrden', 'Clases.nombreClase', 'Divisions.nombreDivision', 'Reinos.nombreReino', 'Especies.idGenero')->where('idEspecie', $r->id)->get();
        ...
        $a_esp = ApendiceCites::join('Especies', 'apendice_cites.idApendiceCITES', '=', 'Especies.idApendiceCITES')->select('apendice_cites.idApendiceCITES', 'apendice_cites.nombreApendiceCITES')->where('Especies.idEspecie', $r->id)->get();
        $p_esp = ProcedenciaEspecie::join('Especies', 'procedencia_especies.idProcedenciaDeLaEspecie', '=', 'Especies.idProcedenciaDeLaEspecie')->where('Especies.idEspecie', $r->id)->get();
        $c_esp = CategoriaMarn::join('Especies', 'categoria_marns.idCategoriaMARN', '=', 'Especies.idCategoriaMARN')->select('categoria_marns.idCategoriaMARN', 'categoria_marns.nombreCategoriaMARN')->where('idEspecie', $r->id)->get();
        $u_esp = CategoriaUICN::join('Especies', 'categoria_u_i_c_ns.idCategoriaUICN', '=', 'Especies.idCategoriaUICN')->select('categoria_u_i_c_ns.idCategoriaUICN', 'categoria_u_i_c_ns.nombreCategoriaUICN')->where('idEspecie', $r->id)->get();

        return view('ingreso.mostrar', compact('esp1_array', 'elementCount', 'append', 'proc', 'cat', 'uicn', 'nc_esp', 'a_esp', 'p_esp', 'c_esp', 'u_esp'));
    ...
    public function guardar_especie(){}
...
//////////////////////////////////      PUBLICO         ///////////////////////////////////////    
    ...
    public function login()
    {
     ...
     public function PUBLIC_busqueda(){
        ...
        return view('publico.Busqueda_avanzada', compact('reino','r_ani','r_bac','r_fun','r_pro','r_pla','r_chro','d_ahua','d_sant','d_sons','d_chal','d_lali','d_ssal','d_cusc','d_caba','d_lapa','d_sanv','d_usul','d_sanm','d_mora','d_laun'));
